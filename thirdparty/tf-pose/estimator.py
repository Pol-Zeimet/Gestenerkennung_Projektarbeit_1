...
class TfPoseEstimator:
    ...
    def __init__(self, graph_path, target_size=(320, 240), tf_config=None, trt_bool=True, ):
        print('using modified estimator')
        ...
        if trt_bool is True:
            print(
                '______________________________________________creating inference graph___________________________________________________________________')
        ...
        print(
            '__________________________________________________importing graphdef________________________________________________________________')
        self.graph = tf.get_default_graph()
        tf.import_graph_def(graph_def, name='TfPoseEstimator')
        self.persistent_sess = tf.Session(graph=self.graph, config=tf_config)

        print(
            '____________________________________________________printing nodes in graph__________________________________________________________')
        for ts in [n.name for n in tf.get_default_graph().as_graph_def().node]:
            print(ts)

        print(
            '____________________________________________________assigning variables__________________________________________________________')
        ...
        print(
            '____________________________________________________warming up__________________________________________________________')

        print(
            '____________________________________________________run 1__________________________________________________________')
        ...
        print(
            '____________________________________________________run 2__________________________________________________________')
        ...
        print(
            '____________________________________________________run 3__________________________________________________________')
        self.persistent_sess.run(
            [self.tensor_peaks, self.tensor_heatMat_up, self.tensor_pafMat_up],
            feed_dict={
                self.tensor_image: [np.ndarray(shape=(target_size[1], target_size[0], 3), dtype=np.float32)],
        ...
        print(
            '____________________________________________________run 4__________________________________________________________')
        self.persistent_sess.run(
            [self.tensor_peaks, self.tensor_heatMat_up, self.tensor_pafMat_up],
            feed_dict={
                self.tensor_image: [np.ndarray(shape=(target_size[1], target_size[0], 3), dtype=np.float32)],
        ...
        print(
            '____________________________________________________INIT COMPLETE____________________________________________________')
        # logs
        if self.tensor_image.dtype == tf.quint8:
            logger.info('quantization mode enabled.')

        # print('____________________________________________________saving engine____________________________________________________')

        # frozen_graph_filename = 'models/test_graph.pb'
        # uff_filename = 'models/tmp.uff'
        # plan_filename = 'models/test_graph.plan'
    ...
    #   'float' # float / half
    # ]
    # subprocess.call(['uff_to_plan.cpp'] + args)
    # print('____________________________________________________saved engine____________________________________________________')
